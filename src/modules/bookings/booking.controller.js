...
const calcFieldPriceBySlots = (startTime, endTime, court) => {
  ...
  if (
    Number.isNaN(startMinRaw) ||
    Number.isNaN(endMinRaw) ||
...
export const calculateBookingPrice = handleAsync(async (req, res, next) => {
  ...
  const { slotCount, fieldAmount, normalHours, peakHours, totalHours } =
    calcFieldPriceBySlots(startTime, endTime, court);

  if (slotCount === 0) {
    return next(
      createError(
        400,
        "Khung giờ không hợp lệ hoặc nằm ngoài giờ hoạt động (06:00–22:00)!"
      )
    );
  }
...
export const createBooking = handleAsync(async (req, res, next) => {
  ...
  const isOfflineMode =
    isOffline === true ||
    isOffline === "true" ||
    roleFromToken === USER_ROLES.ADMIN;
  //Đơn đặt ONLINE (user đặt trên web, thanh toán VNPAY / Momo)
  const isOnlineMode =
    !isOfflineMode &&
    [PAYMENT_METHOD.VNPAY, PAYMENT_METHOD.MOMO].includes(paymentMethod);
  ...
  if (slotCount === 0) {
    return next(
      createError(400, "Khung giờ không hợp lệ hoặc nằm ngoài giờ hoạt động!")
    );
  }
  ...
  if (voucherCode) {
    if (!finalCustomerId) {
      return next(
        createError(400, "Vui lòng chọn khách hàng để áp dụng voucher!")
      );
    }
  ...
  const initialStatus = isOfflineMode
  ...
  return res
    .status(201)
    .json(createResponse(true, 201, "Tạo đơn đặt sân thành công!", booking));
});
...
export const getBookingsByCourt = handleAsync(async (req, res, next) => {
  ...
  return res.json(
    createResponse(true, 200, "Danh sách giờ đã được đặt", bookings)
  );
});
...
export const cancelBooking = handleAsync(async (req, res, next) => {
  ...
  if (user.role === USER_ROLES.USER) {
    ...
    if (booking.status !== BOOKING_STATUS.PENDING) {
      return next(
        createError(400, "Chỉ được hủy đơn đang chờ thanh toán/xác nhận!")
      );
    }
  ...
  if (
    ...
    return next(
      createError(400, "Đơn đang sử dụng/đã hoàn tất, không thể hủy!")
    );
  }
  ...
  return res.json(
    createResponse(true, 200, " Hủy booking thành công!", booking)
  );
});
...
export const confirmBooking = handleAsync(async (req, res, next) => {
  ...
  return res.json(
    createResponse(true, 200, "Xác nhận booking thành công!", booking)
  );
});
...
export const updateBooking = handleAsync(async (req, res, next) => {
  ...
  return res.json(
    createResponse(true, 200, "Cập nhật booking thành công!", booking)
  );
});
...
export const updateBookingTime = handleAsync(async (req, res, next) => {
  ...
  if (!courtId || !date || !startTime || !endTime) {
    return next(
      createError(400, "Thiếu sân, ngày hoặc giờ bắt đầu / kết thúc!")
    );
  }
  ...
  booking.total =
    fieldAmount + (booking.equipmentTotal || 0) - (booking.discountTotal || 0);
  booking.updatedAt = new Date();
  ...
  return res.json(
    createResponse(true, 200, "Cập nhật giờ / sân thành công!", booking)
  );
});
...
export const requestRefund = handleAsync(async (req, res, next) => {
  ...
  if (!user || String(booking.customerId) !== String(user._id)) {
    return next(
      createError(403, "Bạn không có quyền yêu cầu hoàn tiền cho đơn này!")
    );
  }
  ...
  if (!allowStatuses.includes(booking.status)) {
    return next(
      createError(
        400,
        "Chỉ được yêu cầu hoàn tiền cho đơn đang chờ xác nhận hoặc đã hủy!"
      )
    );
  }
  ...
  if (
    ![PAYMENT_STATUS.PAID, PAYMENT_STATUS.PARTIAL].includes(
    ...
    return next(
      createError(
        400,
        "Chỉ được yêu cầu hoàn tiền cho đơn đã thanh toán hoặc đã cọc!"
      )
    );
  }
  ...
  if (!["none", "rejected", undefined, null].includes(booking.refundStatus)) {
    return next(
      createError(
        400,
        "Đơn này đang/đã được xử lý hoàn tiền, không thể gửi lại yêu cầu!"
      )
    );
  }
  ...
  if (!accountNumber || !accountName || !bankName) {
    return next(
      createError(400, "Vui lòng nhập đầy đủ thông tin tài khoản nhận tiền!")
    );
  }
  ...
  return res.json(
    createResponse(
      true,
      200,
      "Gửi yêu cầu hoàn tiền thành công! Vui lòng chờ xử lý.",
      booking
    )
  );
});
...
export const updateRefundStatus = handleAsync(async (req, res, next) => {
  ...
  if (!admin || admin.role !== USER_ROLES.ADMIN) {
    return next(
      createError(403, "Chỉ admin mới được cập nhật trạng thái hoàn tiền!")
    );
  }
  ...
  return res.json(
    createResponse(true, 200, "Cập nhật hoàn tiền thành công!", booking)
  );
});

//*  DASHBOARD ADMIN
export const getAdminDashboardBookings = handleAsync(async (req, res, next) => {
  const [total, pending, confirmed, inUse, completed, cancelled] =
    await Promise.all([
...
export const getBookings = handleAsync(async (req, res, next) => {
  ...
  const baseFilter =
    role === USER_ROLES.ADMIN ? {} : { customerId: req.user._id };
  ...
  return res.json(
    createResponse(true, 200, "Lấy danh sách booking thành công!", bookings)
  );
});
...
export const getBookingsByUser = handleAsync(async (req, res, next) => {
  ...
  return res.json(
    createResponse(
      true,
      200,
      "Lấy danh sách booking của người dùng thành công!",
      bookings
    )
  );
});
...
export const getRetryPaymentInfo = handleAsync(async (req, res, next) => {
  ...
  if (
    ...
    String(booking.customerId?._id) !== String(user._id)
  ) {
  ...
  if (booking.status !== BOOKING_STATUS.PENDING) {
    return next(
      createError(
        400,
        "Chỉ được thanh toán lại cho đơn đang chờ thanh toán/xác nhận!"
      )
    );
  }
...
export const rejectRefundBooking = handleAsync(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(createResponse(true, 200, "Đã từ chối yêu cầu hoàn tiền", booking));
});
...
export const completeRefundBooking = handleAsync(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(
      createResponse(true, 200, "Đã cập nhật hoàn tiền thành công", booking)
    );
});
...
export const addEquipmentsBooking = handleAsync(async (req, res, next) => {
  ...
  if (booking.status !== BOOKING_STATUS.IN_USE) {
    return next(
      createError(400, "Chỉ đơn đang sử dụng mới được thêm thiết bị")
    );
  }
  ...
  const addTotal =
    typeof equipmentTotal === "number"
      ? Number(equipmentTotal)
      : Number(equipmentTotalCalc);
  booking.equipmentTotal = (booking.equipmentTotal || 0) + addTotal;
  //* Cập nhật lại tổng tiền đơn
  booking.total =
    (booking.fieldAmount || 0) + booking.equipmentTotal - booking.discountTotal;
  ...
  return res.json(
    createResponse(true, 200, "Đã thêm thiết bị cho đơn đang sử dụng!", booking)
  );
});
...
export const getBookingEquipmentsDetail = handleAsync(
  async (req, res, next) => {
    const bookingId = req.params.id;
    ...
    return res.json(
      createResponse(
        true,
        200,
        "Lấy danh sách thiết bị của booking thành công!",
        items
      )
    );
  }
